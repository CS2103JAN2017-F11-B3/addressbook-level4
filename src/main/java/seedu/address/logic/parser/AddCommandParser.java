...
public class AddCommandParser {
    ...
    public Command parse(String args) {
        ArgumentTokenizer argsTokenizer = new ArgumentTokenizer(PREFIX_START_DATE, PREFIX_END_DATE, PREFIX_EMAIL,
                PREFIX_GROUP);
        argsTokenizer.tokenize(args);
        try {
            if (!argsTokenizer.getEmpty(PREFIX_START_DATE) && !argsTokenizer.getEmpty(PREFIX_END_DATE)) {
                return new AddCommand(argsTokenizer.getPreamble().get(), argsTokenizer.getValue(PREFIX_END_DATE).get(),
                        argsTokenizer.getValue(PREFIX_START_DATE).get(), argsTokenizer.getValue(PREFIX_EMAIL).get(),
